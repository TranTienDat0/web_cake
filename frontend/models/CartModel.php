<?php
	trait CartModel{		
		...
		//=============
		public function modelcheckoutcustomer(){
               $name = $_POST['name'];
               $email = $_POST['email'];
               ...
               $conn = Connection::getInstance();
               $check = $conn->prepare("select email from shippning where email=:var_email");
               $check->execute(["var_email"=>$email]);
               if($check->rowCount() == 0){
                    
                    $query = $conn->prepare("insert into shippning(name,address,phone,email,note) values(:var_name,:var_address,:var_phone,:var_email,:var_note)");
                    $query->execute(["var_name"=>$name,"var_address"=>$address,"var_phone"=>$phone,"var_email"=>$email,"var_note"=>$note]);
               }else
                    header("location:index.php?controller=account&action=Viewcheckout&notify=error");
        }
	}	
?>